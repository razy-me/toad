...
export class MotionSolver {
   ...
   */
  public evaluateElement(element: MotionSceneElement, time: number): ElementMotionState {
    ...
    if (alongDef && alongDef.targetId) {
      ...
      if (sampler) {
        ...
        // Position at the sampled point, centering the element
        state.x = sampled.x - element.box.width / 2;
        state.y = sampled.y - element.box.height / 2;
